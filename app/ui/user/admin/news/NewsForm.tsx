...
import { useCreateNews } from "@/context/AdminContexts/AdminCreateNewsContext";
...
export default function NewsForm() {
    ...
    const { blocks, submitNews, errors, isSubmitting } = useCreateNews();
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-[75%_1fr] gap-6">
                ...
                <div className="flex flex-col justify-between">
                    <PreviewImage setNotification={setNotification} error={errors?.thumbnail} />
                    <button disabled={isSubmitting} onClick={submitNews} className={`sticky px-4 py-3 bottom-24 right-6 w-fit ml-auto 
                        bg-orange-400 hover:bg-orange-500 transition-all duration-100 ease-in-out cursor-pointer
                        rounded-xl fontA4 text-white ${isSubmitting && "bg-gray-300 opacity-75"}`} type="button">
...
function Title({ error }: { error?: string }) {
    const { title, setTitle } = useCreateNews();
...
function Body({ setNotification, error }: { setNotification: (msg: string) => void, error?: string }) {
    const { addBlock, removeBlock, updateBlock } = useCreateNews();
    const { blocks } = useCreateNews();
    const iconSize = 24;
...
function BlockImage({
    ...
    const [isSubmitting, setIsSubmitting] = useState(false); // bạn có thể truyền từ ngoài vào nếu cần
...
function PreviewImage({
    ...
    const { thumbnail, setThumbnail } = useCreateNews();
