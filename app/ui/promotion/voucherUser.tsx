...
export default function VoucherUser({ data }: { data: Promotion }) {
    ...
    async function toggleVoucherStatus() {
        setNotification("Chỉ quản trị viên mới có thể cập nhật trạng thái mã giảm giá");
    }
        ...
        <div className="px-4 py-2 flex flex-col gap-2 justify-center">
            ...
            <div className="flex justify-between">
                <div className="flex flex-col gap-2">
                    <p className="fontA5">Hiệu lực : {formatDate(data.start_date)} - {formatDate(data.end_date)}</p>
...
export function ToggleButton({ isActive, toggleActive }: { isActive: boolean, toggleActive: () => void }) {
    ...
    return (
        <div
            ...
            className={`
                flex p-1 rounded-full w-[32px] 
                outline-1 
                cursor-pointer
                transition-all duration-300 ease-in-out
                hover:shadow-md
                ${isActive
                    ? "bg-amber-500 outline-amber-400 justify-end"
                    : "bg-white outline-stone-300 justify-start"
                }
            `}
        >
            <div
            ...
            {hovered && (
                <ToolTip>
                    {isActive ? "Có thể sử dụng" : "Chưa kích hoạt"}
                </ToolTip>
            )}
        </div>
    )
