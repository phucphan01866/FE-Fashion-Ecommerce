...
import ReviewService, { ReviewCreateInput } from "@/service/review.service";
...
function PageContent({ product, variantId, orderId }: { product: TypeProduct, variantId: string, orderId: string }) {
    return (
        <div className="container px-4 py-8 flex flex-col gap-6 mx-auto">
            <div className={`addressInfoSection ${sectionCSS} flex flex-col gap-6`}>
                ...
                <Divider />
                <UserInputReview product={product} variantId={variantId} orderId={orderId} />
            </div>
        </div>
    );
}

function UserInputReview({ product, variantId, orderId }: { product: TypeProduct, variantId: string, orderId: string }) {
    const { user } = useAuth();
    ...
    const [previewUrl, setPreviewUrl] = useState<string | null>(null);    // ← Chỉ để preview
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const reviewPayload: ReviewCreateInput = {
                ...
                images: [imageUrl],
                rating: rating || 5, // Giả sử đánh giá 5 sao, có thể thêm UI chọn sao nếu cần
            };

            await ReviewService.addReviews(orderId, [reviewPayload]);
            ...
            setNotification("Gửi đánh giá thành công! Cảm ơn bạn");
    ...
    return (
        <div className="space-y-6 bg-white p-6 rounded-lg border border-gray-200">
            ...
            <div className="flex justify-start items-center gap-12 mb-4">
                <div>
                    <p className="font-semibold">{userProfile?.full_name || userProfile?.name || user?.email || "Khách hàng"}</p>
          ...
          `}
            ...
            ${(reviewText.trim() || imageFile) && !isSubmitting
                            ? "bg-orange-500 text-white hover:bg-primary/90 "
